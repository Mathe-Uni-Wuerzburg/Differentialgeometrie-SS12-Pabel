...
Durch \(u \mapsto \Phi(u) := \big( x^1(u), x^2(u) \big) = \big(\overline u^1(u), \overline u^2(u)\big)\) wird also eine in der Umgebung von \(\mathring u\) zulässige Parametertransformation definiert (mit \(\Phi\left(\mathring u\right) = 0\) nach (a)) [lokaler Umkehrsatz] \par
...
\begin{beweis}[des Hilfssatzes]
 Mit \(A = (a_1, \dots, a_n)\) und \mat{a_k = \sum_{i = 1}^n a_{ik} e_i \, (k = 1, \dots, n)} gilt
 \begin{align*}
...
\uline{Berechnung}
\begin{enumerate}
 \item[a)] \mat{\left[\frac{\partial}{\partial \varepsilon} \,^\varepsilon x_{\varrho}(u)\right]_{\varepsilon = 0} = \frac{\partial}{\partial_u \varrho} V(u) =: V_{\varrho}(u)}
 \item[b)] \mat{\left[\frac{\partial}{\partial \varepsilon} \,^{\varepsilon} g_{\varrho \sigma} (u) \right]_{\varepsilon = 0} = \left[ \frac{\partial}{\partial\varepsilon} \langle \,^\varepsilon x_\varrho(u), \,^\varepsilon x_\sigma (u) \rangle \right]_{\varepsilon = 0} = \langle V_\varrho (u), x_\sigma \rangle + \langle x_\varrho, V_\sigma \rangle (u)}
...
Die ersten \uline{Beispiele} von Minimalflächen wurden, ausgehend von der Eulerschen Darstellung \((u,v) \mapsto x(u,v) = \vxyz{u}{v}{z(u,v)}\) durc Lösen der \uline{Minimalflächengleichung}\index{Minimalflächengleichung}
\[
...
Innerhalb \uline{spezieller Flächenklassen} lassen sich alle Minimalflächen bestimmen. Bis auf Streckung und Spiegelung gilt
\begin{enumerate}
...
\begin{enumerate}
 \item \mat{(g_{\varrho \sigma}) = \lambda^2 \begin{pmatrix}
                                   1 & 0 \\
                                   0 & 1
                                  \end{pmatrix} \text{, wobei } \lambda = \frac{1}{2} |f| \left(1 + |g|^2\right)}
 \item \mat{N = \pi_s^{-1} \circ g = \frac{1}{1 + |g|^2} \vxyz{2 \Real g}{2 \Imag g}{|g|^2 - 1}}
 \item \mat{\Omega := b_{22} + \I b_{12} = -b_{11} + \I b_{12} = f \cdot g'}
\end{enumerate}
...
\begin{bsp} \(\)
 \begin{enumerate}
  ...
  \end{align*}
  \item[b)] Wähle \(\Omega \equiv 1\) (Krümmungslinienparameter), \mat{g(w) = \E^{\I w} \Rightarrow f(w) = \frac{1}{\I \E^{\I w}}} \par
  Dann ist
  \begin{align*}
...
\begin{beweis}[von Satz \ref{satz262}]
 ...
 Für die Hauptkrümmungsrichtungen \mat{Y_\varrho = \frac{x_\varrho}{|x_{\varrho}|} (\varrho = 1,2)} gilt:
 \begin{align*}
...
\begin{definition}
 Eine Regelfläche im \(\IR^3\) heißt \uline{Torse}\index{Torse}, wenn längs jeder erzeugenden Geraden die Tangentialebene fest bleibt.
\end{definition}
...
\begin{beweis}[von Satz \ref{satz263}]
...
\begin{enumerate}
 ...
 Dann gibt es eine spezielle Leitlinie \(t \mapsto g(t) = c(t) + f(t) E(t)\) mit \(\dot g \parallel E\), genannt \uline{Striktionslinie}\index{Striktionslinie} oder \uline{Gratlinie}\index{Gratlinie}. Aus \(\dot g = \dot c + \dot f E + f \dot E\) folgt mit der Tosenbedingung
 \[
...
\subsubsection{Darstellung der Fläche bezüglich der neuen Leitlinie \(g\)}
...
Insgesamt hat man folgende \uline{lokale Klassifikation} der Flächen im \(\IR^3\) mit \(K \equiv 0\):
\begin{enumerate}
...
\begin{beweis}[von Satz \ref{satz265}]
...
\subsection{Rotationsflächen mit konstanter Gaußkrümmung}
...
Sei \((u,v)\mapsto x(u,v)\) eine beliebige Fläche in Krümmungslinienparameter (also mit \(g_{12} = b_{12} = 0\)) \par
Für eine \uline{Parallelfläche}\index{Parallelfläche} \(\,^\varepsilon x = x + \varepsilon N\) gilt dann wegen \(\,^\varepsilon x_\varrho = (1 - \varepsilon K_\varrho) x_\varrho = f_\varrho x_\varrho\) (mit \(f_\varrho > 0\) für kleine \(\varepsilon\))
\begin{align*}
 \,^\varepsilon N &= N \\
